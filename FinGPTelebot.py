...
class FinGPTelebot:
    ...
    def store_sticker(self, message):
        ...
        time_ = self.parse_timestamp(message.date)
        self.last_messages.setdefault(message.chat.id,[]).append((time_, uname, message.sticker.emoji))
    ...
    def store_file(self, message):
        uname = self.parse_username(message.from_user.username)
        time_ = self.parse_timestamp(message.date)
        self.last_messages.setdefault(message.chat.id,[]).append((time_, uname, message.document.file_name))
        print(uname, message.document.file_name)

    def store_message(self, message):
        if len(self.last_messages.setdefault(message.chat.id,[])) >= self.n_messages:
            while len(self.last_messages[message.chat.id]) >= self.n_messages:
        ...
        with open("running_chat_history.json", "a", encoding="utf-8") as f:
            ...
            time_ = self.parse_timestamp(message.date)
            msg = {"time": time_, "chat": message.chat.id, "username": message.from_user.username, "message": message.text}
        ...
        time_ = self.parse_timestamp(message.date)
        print(uname, text)
        self.last_messages.setdefault(message.chat.id,[]).append((time_, uname, text))
    ...
    def send_reply(self, chat_id):
        ...
        prompt = "".join([f"{username}({time_}):{message}\n" for time_, username, message in self.last_messages.setdefault(chat_id,[])])
        # Add "GPT(<time>):"
        prompt += "GPT" + time.strftime("(%H:%M)", time.localtime()) + ":"
        ...
        for line in model_reply:
            ...
            try:
                ...
                self.last_messages.setdefault(chat_id,[]).append((time_ ,"GPT", line))
